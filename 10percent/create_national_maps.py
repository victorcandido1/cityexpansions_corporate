...
def create_national_corporate_map(gdf, df_corporate, df_airports):
    ...
    gdf_corp = gdf_corp.merge(
        df_corporate[['zipcode', 'city_key', 'city_name', 'Corporate_Score', 
        ...
        for idx, row in gdf_corp.iterrows():
            if pd.notna(row.geometry) and pd.notna(row[score_col]):
                ...
                folium.GeoJson(
                    ...
                    tooltip=f"ZIP {row['zipcode']} ({row.get('city_name', 'Unknown')}): {score_name} {row[score_col]:.2f}"
                ).add_to(m)
...
def create_national_intersection_map(gdf, df_household, df_corporate, df_intersection, df_airports):
    ...
    gdf_map = gdf_map.merge(
        df_household[['zipcode', 'city_key', 'city_name', 'Geometric_Score', 
    ...
    # Merge corporate data
    gdf_map = gdf_map.merge(
        df_corporate[['zipcode', 'Corporate_Score',
...
"""
            ...
            folium.GeoJson(
                ...
                tooltip=f"ZIP {row['zipcode']} ({row.get('city_name', 'Unknown')}): {category}"
            ).add_to(m)
    ...
    <div style="position: fixed; 
        ...
        <p style="margin: 5px 0; font-size: 11px; color: #666;">7 Major Metro Areas</p>
    </div>
    '''

...
def calculate_corporate_power_index(df):
    ...
    # Filter ZIPs with actual business activity AND only 7 metros (exclude 'other')
    df_active = df[df['total_employment'] > 0].copy()
    df_active = df_active[df_active['city_key'] != 'other'].copy()
    print(f"\n  Active ZIPs (7 metros, employment > 0): {len(df_active):,}")

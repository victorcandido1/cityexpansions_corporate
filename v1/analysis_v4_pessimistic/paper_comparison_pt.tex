...
\subsection{Multiplicadores de Trafego}

Os multiplicadores de trafego foram derivados de dados historicos do Google Traffic, especificamente usando as estimativas de duracao \textbf{pessimistas} que representam as piores condicoes historicas.

\begin{table}[H]
\centering
\caption{Definicoes dos Multiplicadores de Trafego}
\begin{tabular}{lccl}
\toprule
\textbf{Cenario} & \textbf{Multiplicador} & \textbf{Aumento} & \textbf{Descricao} \\
\midrule
Rapido & 1.00x & Base & Trafego livre \\
Normal & 1.25x & +25\% & Condicoes tipicas diurnas \\
Hora do Pico & 1.68x & +68\% & Horario de pico (7-9h, 17-19h) \\
\rowcolor{lared!20} Pior Caso & 4.68x & +368\% & Congestionamento severo (pessimista) \\
\bottomrule
\end{tabular}
\end{table}
...
O multiplicador de 4.68x para cenarios de pior caso reflete eventos de congestionamento severo como:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Cenarios de pior caso mostram vantagem dramatica do helicoptero}: Com multiplicadores de trafego de 4.68x, o helicoptero economiza 2-3 horas em comparacao com dirigir.
...
\begin{enumerate}
    \item Em \textbf{cenarios de pior caso} (trafego 4.68x), helicoptero economiza:
    \begin{itemize}

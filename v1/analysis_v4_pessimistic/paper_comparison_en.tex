...
\begin{abstract}
This study presents a comprehensive analysis comparing helicopter and car travel times to major airports in New York (JFK) and Los Angeles (LAX) under multiple traffic scenarios. Using real Google Traffic historical data, we evaluate four distinct scenarios with \textbf{city-specific multipliers}: Fast (1.00x), Normal (NY: 1.35x, LA: 1.42x), Rush Hour (NY: 1.69x, LA: 1.77x), and Worst Case (NY: 5.20x, LA: 4.15x based on pessimistic historical data). Our analysis covers 114 ZIP codes (70 in NY, 44 in LA) and demonstrates that helicopter travel provides significant time savings, particularly under severe traffic conditions. New York shows higher worst-case multipliers (5.20x) due to more extreme traffic events, while Los Angeles exhibits higher normal congestion (1.42x). In worst-case scenarios, helicopter travel saves an average of \textbf{147 minutes in New York} and \textbf{143 minutes in Los Angeles}, representing time reductions of 66\% and 64\% respectively. The study also reveals that during worst traffic conditions, average car speeds drop to approximately 12 km/h---barely faster than walking pace.
\end{abstract}
...
\subsection{City-Specific Traffic Multipliers}

Traffic multipliers were derived from Google Traffic historical data, specifically using the \textbf{pessimistic} duration estimates which represent worst-case historical conditions. Importantly, our analysis reveals that \textbf{multipliers differ significantly between cities}:

\begin{table}[H]
\centering
\caption{City-Specific Traffic Multiplier Definitions}
\begin{tabular}{lcccc}
\toprule
\textbf{Scenario} & \textbf{New York} & \textbf{Los Angeles} & \textbf{Description} \\
\midrule
...
\rowcolor{lared!20} Worst Case & \textbf{5.20x (+420\%)} & \textbf{4.15x (+315\%)} & Severe congestion (pessimistic) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key insight:} New York exhibits more extreme worst-case conditions (5.20x) due to unpredictable events such as accidents, weather, and special events. Los Angeles, while having higher baseline congestion (1.42x normal), shows less extreme worst-case scenarios (4.15x) due to more distributed traffic patterns across its extensive highway network.

\subsection{Traffic Multiplier Calculation Methodology}
...
We collected traffic data from Google's Distance Matrix API using the \textbf{pessimistic} traffic model, which returns the worst-case historical travel times. For each route:

\begin{enumerate}
    \item \textbf{API Request}: Send request with origin (ZIP code centroid), destination (JFK/LAX), and \texttt{traffic\_model=pessimistic}
    \item \textbf{Response Data}: Receive \texttt{duration} (baseline without traffic) and \texttt{duration\_in\_traffic} (with historical pessimistic traffic)
    \item \textbf{Calculate Multiplier}: Compute ratio = \texttt{duration\_in\_traffic} / \texttt{duration}
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../diagrams/multiplier_methodology_en.png}
\caption{Traffic Multiplier Calculation Methodology}
\label{fig:methodology}
\end{figure}

\subsubsection{Numerical Examples}

\textbf{Example 1: New York (Manhattan to JFK)}
\begin{itemize}
    \item Route: Upper East Side (ZIP 10021) $\rightarrow$ JFK Airport
    \item Google API baseline duration: 47 minutes
    \item Google API pessimistic duration: 245 minutes
    \item Calculated multiplier: $245 \div 47 = 5.21\times$
\end{itemize}

\textbf{Example 2: Los Angeles (Beverly Hills to LAX)}
\begin{itemize}
    \item Route: Beverly Hills (ZIP 90210) $\rightarrow$ LAX Airport
    \item Google API baseline duration: 36 minutes
    \item Google API pessimistic duration: 149 minutes
    \item Calculated multiplier: $149 \div 36 = 4.14\times$
\end{itemize}

\subsubsection{City-Level Aggregation}

The final city multipliers were calculated by averaging all route multipliers within each city:

\begin{table}[H]
\centering
\caption{Multiplier Calculation Summary}
\begin{tabular}{lccc}
\toprule
\textbf{City} & \textbf{Routes Analyzed} & \textbf{Avg. Pessimistic Multiplier} \\
\midrule
New York & 52 routes & \textbf{5.20x} \\
Los Angeles & 44 routes & \textbf{4.15x} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../diagrams/google_api_flow_en.png}
\caption{Google Distance Matrix API Data Flow}
\label{fig:api_flow}
\end{figure}

The \textbf{Normal} and \textbf{Rush Hour} multipliers were interpolated based on the ratio between typical daytime and peak hour traffic patterns observed in the data.
...
Understanding the variability of travel times is crucial for risk assessment and travel planning. We analyzed the variance, standard deviation, and coefficient of variation (CV) across all ZIP codes.

\subsection{Travel Time Variability by Scenario}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../variance_charts_en/fig1_std_by_scenario.png}
\caption{Standard Deviation of Travel Times by Scenario}
\label{fig:variance_std}
\end{figure}

Key observations:
\begin{itemize}
    \item Car travel time variance increases dramatically with traffic severity
    \item Helicopter time variance remains relatively stable across scenarios
    \item In worst case: Car Std=102 min vs Helicopter Std=35-90 min
\end{itemize}

\subsection{Coefficient of Variation Analysis}

The coefficient of variation (CV = $\sigma / \mu \times 100\%$) measures relative variability:

\begin{table}[H]
\centering
\caption{Coefficient of Variation by City and Scenario (Worst Case)}
\begin{tabular}{lcccc}
\toprule
\textbf{City} & \textbf{Car Mean} & \textbf{Car Std} & \textbf{Car CV} & \textbf{Heli CV} \\
\midrule
New York & 221 min & 102 min & 46.3\% & 121.1\% \\
Los Angeles & 222 min & 105 min & 47.3\% & 44.3\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../variance_charts_en/fig2_cv_comparison.png}
\caption{Coefficient of Variation Comparison: Car vs Helicopter}
\label{fig:variance_cv}
\end{figure}

\subsection{Regional Variance Analysis}

Different regions show markedly different travel time variability:

\begin{table}[H]
\centering
\caption{Regional Travel Time Variance (Worst Case Scenario)}
\begin{tabular}{llcccc}
\toprule
\textbf{City} & \textbf{Region} & \textbf{N} & \textbf{Mean} & \textbf{Std} & \textbf{CV} \\
\midrule
NY & Manhattan & 27 & 152 min & 12 min & \textbf{7.9\%} \\
NY & Brooklyn & 5 & 158 min & 37 min & 23.8\% \\
NY & New Jersey & 14 & 260 min & 57 min & 21.8\% \\
NY & Long Island & 19 & 299 min & 143 min & \textcolor{red}{47.8\%} \\
\midrule
LA & Pasadena & 3 & 198 min & 10 min & \textbf{4.9\%} \\
LA & Beverly Hills & 10 & 106 min & 23 min & 21.7\% \\
LA & Orange County & 24 & 278 min & 99 min & \textcolor{red}{35.4\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../variance_charts_en/fig4_regional_cv_heatmap.png}
\caption{Regional Travel Time Variability Heatmap}
\label{fig:variance_heatmap}
\end{figure}

\textbf{Key Insight:} Manhattan and Pasadena show the \textbf{lowest variance} (CV $<$ 10\%), making travel times more predictable. Long Island and Orange County show the \textbf{highest variance} (CV $>$ 35\%), indicating greater uncertainty in travel times.

\subsection{Distribution Analysis}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../variance_charts_en/fig3_boxplots_variance.png}
\caption{Travel Time Distribution by Scenario (Boxplots with Standard Deviation)}
\label{fig:variance_boxplot}
\end{figure}

\subsection{Correlation: Travel Time vs Savings}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../variance_charts_en/fig5_scatter_mean_vs_savings.png}
\caption{Scatter Plot: Car Travel Time vs Helicopter Savings}
\label{fig:variance_scatter}
\end{figure}

The strong positive correlation ($R^2 > 0.9$) indicates that the longer the car journey, the greater the helicopter advantage. This relationship is nearly linear, confirming that helicopter benefits scale proportionally with distance.
...
\begin{enumerate}
    \item \textbf{Worst-case scenarios show dramatic helicopter advantage}: With city-specific traffic multipliers (NY: 5.20x, LA: 4.15x), helicopter travel saves 2-3 hours compared to driving.
...
\begin{enumerate}
    \item In \textbf{worst-case scenarios} (NY: 5.20x, LA: 4.15x traffic), helicopter saves:
    \begin{itemize}

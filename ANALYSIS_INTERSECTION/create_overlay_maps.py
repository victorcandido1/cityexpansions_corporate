...
# Add root directory to path to import qualitative data
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
try:
    from qualitative.map_utils import resolve_fbo_coordinates, add_fbo_markers, CITY_TO_REGION
except ImportError:
    print("Warning: Could not import qualitative data")
    resolve_fbo_coordinates = lambda x: {}
    add_fbo_markers = lambda m, c, r: None
    CITY_TO_REGION = {}
...
def load_data():
    ...
    try:
        ...
        df_airports = df_airports[['Name', 'Facility Type', 'Ownership', 'Use', 
                                   'ARP Latitude DD', 'ARP Longitude DD', 'City', 'State Id', 'Loc Id']]
...
# CREATE OVERLAY MAP
# =============================================================================
def create_overlay_map(gdf, df_household, df_corporate, df_intersection, df_airports, city_key, config, fbo_coords=None):
...
"""
    ...
    if fbo_coords:
        city_slug = city_key
        region_name = CITY_TO_REGION.get(city_slug)
        if region_name:
            add_fbo_markers(m, fbo_coords, region_filter=region_name)
...
if __name__ == '__main__':
    ...
    # Resolve FBO coordinates
    print("Resolving FBO Coordinates...")
    fbo_coords = resolve_fbo_coordinates(df_airports)
    print(f"  Found {len(fbo_coords)} FBO locations.")
    ...
    for city_key, config in CITIES.items():
        ...
        m = create_overlay_map(gdf, df_household, df_corporate, df_intersection, 
                              df_airports, city_key, config, fbo_coords)
